...
function `greeks()` allows vectorized calculation of greeks for any
option pricing calculation based on the standard Black-Scholes or
barrier option
inputs.^[In addition, `greeks2` performs the same calculations with a different calling convention, and `bsopt()` simultaneously computes prices and Greeks for European calls and puts.]

As an example, the following calculation will produce identical
...
x <- binomopt(43, 40, .3, .08, 1, 0, 3, putopt=TRUE, returntrees=TRUE)
x
