...
\item Greek calculations are perhaps more convenient than in some
...
The |bsopt| function provides all option Greeks, vectorized, for both puts and
calls. See Figure \ref{fig:allgreeks}. 
...
Greeks for Black-Scholes style and barrier options can be computed
using the |greeks| function, which is a wrapper for any pricing
function that returns the option price and which uses the default
naming of inputs.\footnote{At the moment, there are two alternative
  functions:
  \begin{itemize}
  \item For European calls and puts, the bsopt function by default
    produces Greeks in addition to prices.
\item The greeks2 function takes as arguments the name of the pricing
  function and then its inputs as a list.
\end{itemize}
}
...
custom pricing functions and multiple options, including spreadsYou can also compute for custom pricing functions. Here is the formula
for a prepaid contract that pays $S_{T}^{a}$ at time $T$:
<<>>=
...
You can compute its Greek by wrapping it with the greeks function:
<<>>=
greeks(powercontract(s=40, v=.08, r=0.08, tt=0.25, d=0, a=2))
@ 

For a spread, simply define the value of the spread as a function and
then compute the greeks for this function:
<<>>=
bullspread <- function(s, k1, k2, v, r, tt, d, fn) {
    fn(s, k1, v, r, tt, d) - fn(s, k2, v, r, tt, d)
}
greeks(bullspread(40, 40, 45, .3, .08, 1, 0, bscall))
@ 

The Greeks function is vectorized, so you can do this:
<<>>=
