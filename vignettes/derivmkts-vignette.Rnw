...
setting \lstinline{priceonly=FALSE}, the function returns both the option price
...
the value of a  prepaid contract that pays $S_{T}^{a}$ at time $T$ is
given by the \lstinline{powercontract()} function:
<<>>=
...
release.\footnote{At this time the \lstinline{greeks()} function will
  ...
  random number seed, resulting in price variation due solely to
  random variationso there will be variation in the option value both
  because the inputs are different and because the random number seed
  has changed. In a future release I hope to address this. For the
  curious,
  \href{http://stackoverflow.com/questions/14324096/setting-seed-locally-not-globally-in-r}{this
    Stackoverflow post} discusses one issue that arises in this
  context.}
...
The functions return both call and put prices, which plays havoc with
the \lstinline{greeks()} function. For now the workaround is to create
a new function that returns one price:
<<>>=
geomavgpricecall <- function(s, k, v, r, tt, d, m) geomavgprice(s, k, v, r, tt, d, m)['Call']
greeks(geomavgpricecall(40, 40, .3, .08, .25, 0, 3))

@ 

Be aware that this calculation is not vectorized.
