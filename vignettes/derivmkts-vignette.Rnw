...
\emph{Derivatives Markets}. The material has an educational
...
The important things to know are that the dividend yield must be
positive if the option is a call, and the risk-free rate must be
positive if the option is a put. If either of these is not true, the
option is never exercised and there is no price. 

The pricing formula can return, in a list, both the option price and
the stock price at which the option is optimally exercised (the
