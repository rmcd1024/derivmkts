...
bibliography: "/home/rmcd/texmf/bibtex/bib/rmcd.bib"
header-includes:
  \usepackage{tikz}
...
the package.^[See @black/scholes:73 and
  @merton:73-bell.]  The functions `bscall`, `bsput`, and
...
These may be deprecated in the future. `greeks2` is
...
```{r allgreeks, fig.cap='All option Greeks for a call and a put, computed using the greeks function', out.height='6in', out.width='5in'}
...
that can be used for Monte Carlo or any other purpose. 
...
```{r, fig.cap='Five simulated paths for the same stock, no jumps.' }
...
We can do the same exercise adding jumps:

```{r, fig.cap='Five simulated paths for the same stock, which can jump.'}
s0 <- 100; v <- 0.3; r <- 0.10; d <- 0; tt <- 1
...
45%., with the covariance matrix given by `v`. Compare the estimated
...
vc <- matrix(c(1, .4, -.3,
              ...
              -.3, .25, 1), nrow = 3) 
v <- vi %*% vc %*% vi
s <- simprice(s0 = s0, v = v, r = r, tt = 1, d = d, trials = 1,
              periods = 365, jump = FALSE, long = TRUE)
threestocks <- s %>%
    filter(trial == 1) %>%
    group_by(asset) %>%
...
```{r binomplot3, fig.cap="Binomial plot when nstep is 40.\\label{fig:binomplot3}"}
d <- 0.06
...
```{r binomplot4, fig.cap="Binomial plot when nstep is 40 using the argument ylimval to focus on a subset.\\label{fig:binomplot4}"}
d <- 0.06
